...
namespace EnvironmentMonitor.WebApi.Controllers
{
    ...
    public class DeviceCommandsController : ControllerBase
    {
        ...
        public async Task<Dictionary<int, string>> GetDeviceAttributes([FromRoute] string deviceIdentifier)
        {
            _logger.LogInformation($"API: Getting attributes for device: {deviceIdentifier}");
            return await _deviceCommandService.GetDeviceAttributes(deviceIdentifier);
        }
    }
}
